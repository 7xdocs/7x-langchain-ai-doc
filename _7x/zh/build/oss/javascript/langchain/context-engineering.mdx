...
LangChain的[中间件](/oss/javascript/langchain/middleware)是底层机制，使得使用LangChain的开发者能够实际进行上下文工程。
...
对于**持久**更新（修改状态，如[生命周期上下文](#summarization)中的摘要示例），请使用生命周期钩子，如 `before_model` 或 `after_model` 来永久更新对话历史。有关更多详细信息，请参阅[中间件文档](/oss/javascript/langchain/middleware)。
</Note>
...
正如您在[模型上下文](#model-context)和[工具上下文](#tool-context)中所见，[中间件](/oss/javascript/langchain/middleware)是使上下文工程切实可行的机制。中间件允许您钩入智能体生命周期中的任何步骤，并：
