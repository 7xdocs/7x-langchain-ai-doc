...
LangChain 代理使用 [LangGraph 持久化](/oss/javascript/langgraph/persistence#memory-store) 来实现长期记忆。这是一个较为高级的主题，需要具备 LangGraph 相关知识才能使用。

## 记忆存储

LangGraph 将长期记忆以 JSON 文档的形式存储在 [存储库](/oss/javascript/langgraph/persistence#memory-store) 中。
