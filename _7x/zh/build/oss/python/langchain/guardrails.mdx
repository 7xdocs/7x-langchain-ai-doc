...
您可以使用[中间件](/oss/python/langchain/middleware)在策略性节点拦截执行过程——例如在智能体启动前、完成后，或在模型调用与工具调用前后——来实现防护栏。
...
<Tip>
    有关实现审批工作流的完整详细信息，请参阅[人工介入循环文档](/oss/python/langchain/human-in-the-loop)。
</Tip>
...
- [人工介入循环](/oss/python/langchain/human-in-the-loop) - 为敏感操作添加人工审核
