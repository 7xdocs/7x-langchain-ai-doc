...
在 v1.0 之前，我们推荐使用 @[`langgraph.prebuilt.create_react_agent`][create_react_agent] 来构建智能体。现在，我们推荐您使用 @[`langchain.agents.create_agent`][create_agent] 来构建智能体。
...
| [提示](#提示) | 参数重命名为 @[`system_prompt`]，动态提示使用中间件 |
| [模型前钩子](#模型前钩子) | 被带有 `before_model` 方法的中间件取代 |
| [模型后钩子](#模型后钩子) | 被带有 `after_model` 方法的中间件取代 |
| [自定义状态](#自定义状态) | 仅支持 `TypedDict`，可以通过 @[`state_schema`] 或中间件定义 |
...
<Note>
通过中间件定义自定义状态优于通过 @[`create_agent`] 上的 @[`state_schema`] 定义，因为它允许您将状态扩展概念上限定在相关的中间件和工具范围内。
...
动态模型选择允许您根据运行时上下文（例如，任务复杂性、成本约束或用户偏好）选择不同的模型。在 [`langgraph-prebuilt`](https://pypi.org/project/langgraph-prebuilt) 的 v0.6 中发布的 @[`create_react_agent`] 支持通过传递给 `model` 参数的可调用对象进行动态模型和工具选择。
...
@[`create_agent`] 的 @[`tools`][create_agent(tools)] 参数接受一个列表，包含：
...
在 v1 中，消息获得了与提供者无关的标准内容块。通过 @[`message.content_blocks`][content_blocks] 访问它们，以获取跨提供者的一致、类型化视图。现有的 @[`message.content`][BaseMessage(content_blocks)] 字段对于字符串或提供者原生结构保持不变。

### 变更内容

- 消息上新增 @[`content_blocks`][BaseMessage(content_blocks)] 属性，用于规范化内容
- 标准化的块形状，记录在 [消息](/oss/langchain/messages#standard-content-blocks) 中
...
更多详细信息，请参阅内容块 [参考](/oss/langchain/messages#content-block-reference)。
...
聊天模型调用的返回类型签名已从 @[`BaseMessage`] 修正为 @[`AIMessage`]。实现 @[`bind_tools`][BaseChatModel.bind_tools] 的自定义聊天模型应更新其返回签名：
