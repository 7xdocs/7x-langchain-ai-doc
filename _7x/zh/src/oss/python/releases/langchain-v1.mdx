...
@[`create_agent`] 是 LangChain 1.0 中构建智能体的标准方式。它提供了比 @[`langgraph.prebuilt.create_react_agent`][create_react_agent] 更简单的接口，同时通过使用 [中间件](#middleware) 提供了更大的自定义潜力。
...
优秀的智能体需要 [上下文工程](/oss/langchain/context-engineering)：在正确的时间将正确的信息传递给模型。中间件通过可组合的抽象，帮助您控制动态提示、对话摘要、选择性工具访问、状态管理和防护栏。
...
因为 @[`create_agent`] 构建于 [LangGraph](/oss/langgraph) 之上，您自动获得了对长期运行和可靠智能体的内置支持，通过：
...
- **向后兼容**：标准内容可以 [延迟加载](/oss/langchain/messages#standard-content-blocks)，因此没有相关的破坏性更改
...
请参阅我们的 [迁移指南](/oss/migrate/langchain-v1) 以获取帮助，将您的代码更新到 LangChain v1。
