...
<Note>
    如果您在工具内部添加了 @[`get_stream_writer`]，那么您将无法在 LangGraph 执行上下文之外调用该工具。
</Note>
:::
...
这在[多智能体](/oss/langchain/multi-agent)系统中非常有用，可以控制哪些智能体流式传输其输出。

请参阅[模型](/oss/langchain/models#disable-streaming)指南以了解如何禁用流式传输。
